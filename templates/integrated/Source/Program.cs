...
bld.Services
   ...
   .AddSingleton(await InitDatabase(bld.Configuration["Database:Name"]!))
...
async Task<DB> InitDatabase(string dbName)
{
    //var db = await DB.InitAsync(dbName, MongoClientSettings.FromConnectionString("mongodb://admin:password@localhost:27017/?authSource=admin"));
    var db = await DB.InitAsync(dbName);
    await db.MigrateAsync();
    await JobRecord.CreateIndexesAsync(db);
    await Notification.Initialize(db);
